...
I chose conda's venv: conda 24.9.2
i had it setup from a previous class to provide me with django and tools.
>: conda list
...
The attributes that determines this handling logic are found in one Model (Card).
...
##################################
Views
	The most important view will be the card detail view. it's path looks like this: 
	'card/<str:card_token>'.

	This should grab the record in the Card associated with `card_token`
	From there, 4 things may occur:
		1) Redirect the browser to their cards url
		2) Ask to activate the card
		3) Redirect the browser to the owners profile
		4) Show an error if tag not found not found

		For 2, i can manually render the activate card page with the associated  card in the context
	...
	[Sun Sep 21] 
	Profile view:
		-I've added the Profile model attrs to profile_html template if those attrs have values 
		- Now I need to configure static and media to display imgs from the Profile model.  As of now, my media's root is the base directory
		https://stackoverflow.com/questions/5517950/django-media-url-and-media-root
		- Let's get back to card detail view and see if we can properly redirect that specific case to the profile detail view. 
			-- I'll grab the Card's user, and the Profile slug of the user, and send that slug to the profile detail view

	Activate Card View /card/<str:card_token>:
		This is a view that is visible if any only if the card with card_token is not owned.
		If a card is owned, just pass the token to the card_detail view
		The activation template will show the design of the card, the token, and a button asking to claim this card.

		When that button is clicked, the browser is taken to stripe for purchasing. Once a payment intent is received at some url/view combo, update card by setting card.user. I'll head to the feature branch  and play with sending data to and from stripe. The cards are one of a types: a preset, or a custom design (There are several types of presets). I'm wondering if I should alter my Card model and add a card_type foreign key. Theoretically speaking I'll be able to link the following attribute(s): stripe payment link, default designs, etc

		As a company, it'd be cool to show the customers the preset design they chose, at the beginning of the activation, in the Stripe checkout.

		New Model proposition:
		...
		Hmmm, I would be able to access the correct payment link from a Card model... but what if this is overkill? To determine that, I'd have to experiment with stripe's post checkout ui. I know that I can send metadata like a card_token... enough talk. To the feature_stripe branch we go (right after this git stash)
			...
			Alright. This (https://testdriven.io/blog/django-stripe-tutorial/) says that embedding Stripe into Django requires 4 steps.  
				...
				4) Check webhook for successful payment		

			The current flow for stripe:
				1) grab publishable key 
				2) Click activate card at activate card view
				3) create a checkout session (/create-checkout-session)
				3) redirect to success/cancelled 
				4) Listen to webhook for checkout.session.completed [evt_1SA2RiAAVj3eEQONfu6cfCiO](/webhook)

				While this all works, I need to have access to the card_token and user_id variables at the /webhook/ view to let the system know a card is being claimed.
				Stripe can hold these two! Which means I'll need to have the two ready before creating a stripe checkout session. I'll use request session to hold the info, but the question is when. 
		...
		To handle Stripe, I've implemented 3 new csrf_exempt Views:
		(urlpattern)-> viewfunction(params):

			# Grabs Publishable Key 
			# Returns JSONResponse 
			- (config/)-> stripe_config(request)
			
			# Generates a Stripe checkout session id
			# Customize the checkout exp here
			# Relies on request.session keys
			- (create-checkout-session/)-> create_checkout_session(request)

			# Listens for payment confimation
			...
			Stripe was not having it with those Stripe test keys. For now I've enabled them to be pushed since I'm dealing with a test account.

		[Thu Sep 25 2025]
		- Login View:
			- modifies the django auth LoginView
			- uses get_success_url() to handle the case where a card is being activated
			- 
...
Templates
	...
	- activate.html : I'm adding the next parameter to the activate card button's url. It will normally point to the login/ view (unless a user is already signed in... Then it'd point to ). This lets me control the success url over at the login view. If the next parameter is found, the success url of the login form will be the url pathway 
